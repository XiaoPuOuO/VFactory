const SECRET_PAYLOAD_KEY_RE =
  /(api[-_]?key|access[-_]?token|auth(?:_?token)?|authorization|bearer|secret|passwd|password|credential|jwt|private[-_]?key|cookie|connectionstring|webhook|signing[-_]?key|hmac|token|refresh[-_]?token|client[-_]?secret|session|csrf|otp|pin|ssn|iban)/i;
