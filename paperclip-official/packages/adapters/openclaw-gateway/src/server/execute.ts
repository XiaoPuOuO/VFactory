...
function buildWakeText(payload: WakePayload, paperclipEnv: Record<string, string>): string {
  ...
  const lines = [
    ...
    "   - If the returned issues list is empty:",
    "     - GET /api/companies/$PAPERCLIP_COMPANY_ID/goals",
    "     - Select one goal whose status is active or planned (ignore achieved/cancelled).",
    "     - Create a new issue for that goal by calling POST /api/companies/$PAPERCLIP_COMPANY_ID/issues with:",
    "       {\"goalId\":\"<goal.id>\", \"title\":\"Goal: <goal.title>\", \"description\": <goal.description if present>, \"status\":\"todo\", \"priority\":\"medium\", \"assigneeAgentId\":\"$PAPERCLIP_AGENT_ID\"}",
    "     - Treat the newly created issue as issueId and continue with step 3 (checkout + execute).",
    "     - If no suitable goal exists, complete the run without issue work.",
    ...
    "6) Project creation consistency:",
    "   - If you create or initialize a new product/app codebase in the filesystem AND you want it to appear in this company as a \"Project\" in the Board UI, you MUST also create the project record via POST /api/companies/$PAPERCLIP_COMPANY_ID/projects.",
    "   - Minimal payload example: {\"name\":\"<project name>\", \"status\":\"backlog\"} (add \"description\", \"goalIds\" when available).",
    "   - If you also want to register an execution workspace, call POST /api/projects/{projectId}/workspaces with at least one of {\"cwd\":\"<absolute cwd>\"} or {\"repoUrl\":\"<url>\"}.",
    "",
    ...
    "",
    "Useful endpoints for project work:",
    "- POST /api/companies/{companyId}/projects (create project record for the company)",
    "- POST /api/projects/{projectId}/workspaces (create a workspace reference for execution)",
    "",
