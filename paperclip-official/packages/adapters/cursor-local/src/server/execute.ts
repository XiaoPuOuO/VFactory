...
const PAPERCLIP_SKILLS_CANDIDATES = [
  ...
  path.resolve(__moduleDir, "../../../../../skills"),
  // dev: src/server/ -> repo root/.cursor/skills/
...
function renderBrowserUseApiNote(env: Record<string, string>): string {
  ...
  return [
    ...
    "If you need browser automation, use Paperclip Browser-Use Tool API endpoints under /api/tools/browser-use/*.",
    "Flow: sessions/start -> navigate/state/click/type/extract/screenshot -> sessions/close.",
    "",
    "",
  ].join("\n");
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (chatRoomId) {
    ...
    // Enable adapter-managed chat replies so agents do not need to call the chat API directly.
  ...
  const prompt = `${instructionsPrefix}${paperclipEnvNote}${browserUseApiNote}${chatModePrefix}${renderChatProjectScopeNote(env)}${crossChatBlock}${roomEarlierBlock}${chatHistoryBlock}${renderedPrompt}`;
