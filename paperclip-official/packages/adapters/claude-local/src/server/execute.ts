...
function renderBrowserUseApiNote(env: Record<string, string>): string {
  ...
  return [
    ...
    "When browser automation is needed, call Paperclip Browser-Use Tool API endpoints.",
    "Use /api/tools/browser-use/sessions/start first, then /navigate|/state|/click|/type|/extract|/screenshot, and finally /sessions/close.",
    "",
    "",
  ].join("\n");
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const prompt = `${browserUseApiNote}${renderChatProjectScopeNote(env)}${appendAgentCapabilitiesToRenderedPrompt(
    promptTemplate,
