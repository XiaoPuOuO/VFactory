...
function resolveCodexBillingType(adapterType: string, env: Record<string, string>): "api" | "subscription" {
  if (adapterType === "codex_remote") return "api";
  if (adapterType === "codex_local") return "subscription";
...
function renderBrowserUseApiNote(env: Record<string, string>): string {
  ...
  return [
    ...
    "Use Paperclip Browser-Use Tool API for browser automation (do not invent custom endpoints).",
    `  curl -s -X POST -H "Authorization: Bearer $PAPERCLIP_API_KEY" -H "Content-Type: application/json" \\`,
    `    "$PAPERCLIP_API_URL/api/tools/browser-use/state" \\`,
    `    -d '{"agentId":"'$PAPERCLIP_AGENT_ID'","runId":"'$PAPERCLIP_RUN_ID'","toolInput":{"sessionId":"<session-id>","includeScreenshot":false}}'`,
    "Available actions:",
    "  /api/tools/browser-use/sessions/start, /navigate, /click, /type, /state, /extract, /screenshot, /sessions/close",
    "",
    "",
  ].join("\n");
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const billingType = resolveCodexBillingType(agent.adapterType ?? "", env);
  ...
  const prompt = `${browserUseApiNote}${renderChatProjectScopeNote(env)}${instructionsPrefix}${renderedPrompt}`;
