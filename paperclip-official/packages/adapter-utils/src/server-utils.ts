...
/** Matches `{{ agent.capabilities }}` with optional whitespace inside the braces. */
const AGENT_CAPABILITIES_TEMPLATE_RE = /\{\{\s*agent\.capabilities\s*\}\}/;

/**
 * When the heartbeat prompt template omits `{{ agent.capabilities }}`, append the DB
 * `capabilities` string so org-chart "能力描述" still reaches the model. Skips if the
 * template already references the placeholder (renderTemplate has expanded it) or if
 * the rendered prompt already contains the same text.
 */
export function appendAgentCapabilitiesToRenderedPrompt(
  templateSource: string,
  renderedPrompt: string,
  agent: unknown,
): string {
  if (AGENT_CAPABILITIES_TEMPLATE_RE.test(templateSource)) {
  ...
  const caps =
    ...
    typeof (agent as { capabilities: unknown }).capabilities === "string"
      ? (agent as { capabilities: string }).capabilities.trim()
      : "";
  if (!caps) {
    return renderedPrompt;
  }
  if (renderedPrompt.includes(caps)) {
    return renderedPrompt;
  }
  return `${renderedPrompt}\n\n【能力描述／Capabilities】\n${caps}\n`;
}
